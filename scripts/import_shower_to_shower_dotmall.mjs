...
const COLLECTION_HANDLE = "shower-to-shower";
const BRAND_ID = "shower-to-shower";
const BRAND_NAME = "Shower to Shower";
...
function normalizeProduct(source) {
  ...
  const id = `shower-to-shower-${source.id}`;
  ...
  return {
    ...
    tags: Array.from(new Set(["shower-to-shower", "dotmall", ...tags.map((t) => slugify(t)).filter(Boolean)])),
...
async function main() {
    ...
    .set(
      {
        name: BRAND_NAME,
        logoURL: null,
        description: "Imported from Dotmall Shower to Shower collection.",
  ...
  const outputPath = path.join(outputDir, "shower-to-shower-products.json");
