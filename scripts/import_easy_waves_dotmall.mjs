...
const COLLECTION_HANDLE = "easy-waves-1";
const SOURCE_BASE_URL = "https://dotmall.co.za";
const BRAND_ID = "easy-waves";
...
async function fetchAllCollectionProducts() {
  ...
  for (let page = 1; page <= 40; page += 1) {
    ...
    if (!res.ok) {
      throw new Error(`Failed to fetch collection page ${page}: ${res.status} ${res.statusText}`);
    }
...
function productFromShopify(source) {
  ...
  return {
    ...
    tags: Array.from(new Set(["easy-waves", "dotmall", ...tags.map((t) => slugify(t)).filter(Boolean)])),
...
async function main() {
  ...
  const sourceProducts = await fetchAllCollectionProducts();
  if (sourceProducts.length === 0) {
    console.log("No products found in source collection.");
    return;
  }

  const normalizedProducts = sourceProducts.map(productFromShopify);
    ...
    .set(
      {
        name: "Easy Waves",
        logoURL: null,
        description: "Imported from Dotmall Easy Waves collection.",
        countryOfOrigin: "South Africa",
        website: `${SOURCE_BASE_URL}/collections/${COLLECTION_HANDLE}`,
        isActive: true,
        isFeatured: false,
  ...
  const outputPath = path.join(outputDir, "easy-waves-products.json");
