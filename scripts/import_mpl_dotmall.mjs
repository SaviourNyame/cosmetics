...
const COLLECTION_HANDLE = "mpl";
const BRAND_ID = "mpl";
const BRAND_NAME = "MPL";
...
function normalizeProduct(source) {
  ...
  return {
    ...
    tags: Array.from(new Set(["mpl", "dotmall", ...tags.map((t) => slugify(t)).filter(Boolean)])),
...
async function main() {
    ...
    .set(
      {
        name: BRAND_NAME,
        logoURL: null,
        description: "Imported from Dotmall MPL collection.",
  ...
  const outputPath = path.join(outputDir, "mpl-products.json");
