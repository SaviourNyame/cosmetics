...
const COLLECTION_HANDLE = "sofnfree";
const BRAND_ID = "softnfree";
const BRAND_NAME = "Softnfree";
...
function normalizeProduct(source) {
  ...
  return {
    ...
    tags: Array.from(new Set(["softnfree", "dotmall", ...tags.map((t) => slugify(t)).filter(Boolean)])),
...
async function main() {
    ...
    .set(
      {
        name: BRAND_NAME,
        logoURL: null,
        description: "Imported from Dotmall Softnfree collection.",
  ...
  const outputPath = path.join(outputDir, "softnfree-products.json");
